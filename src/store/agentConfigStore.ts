...
export const DEFAULT_CONFIGS: Record<AgentType, AgentConfig> = {
    ...
    novel_writer: {
        ...
        systemPrompt: `你是一位互动文字冒险游戏的叙述者（Narrator）。请根据用户的选择和要求，推进故事发展。
          ...
          3. **物品与状态约束**：
             - 主角只能使用【主角状态】中“物品栏”列出的物品，或当前场景环境中合理存在的物品（如医务室的绷带、路边的石头）。
             - 严禁让主角使用凭空出现的物品（如在荒郊野外凭空变出急救包）。
             - 如果主角处于“受伤”状态（见【主角状态】），请在描写中体现伤痛对行动的影响。
          4. 叙事风格：
          ...
          5. 字数控制：
             - 严格控制在 100 - 300 字之间。
             - 不要过于冗长，保持节奏紧凑。
          6. **仅输出故事正文**。严禁包含任何解释、前言、后记、元数据或“修改说明”。`
    },
    option_generator: {
        model: 'deepseek-chat',
        temperature: 0.7,
    ...
    expert_critique: {
        ...
        2. **物品与状态检查（重点）**：
           - 检查主角使用的物品是否合理：是否在【主角状态】的“物品栏”中？或者符合当前场景环境？
           - 如果主角使用了未持有且环境中不可能存在的物品，请务必指出为逻辑漏洞。
           - 检查主角的行为是否符合当前的身体状态（例如腿部骨折却能飞奔）。
        3. **响应检查**：检查草稿是否响应了用户的选择。
        4. **节奏与范围检查**：
    ...
    character_recorder: {
        ...
        1. **识别人物**：找出最新章节中登场或被提及的所有人物。
           - **主角识别（关键）**：你需要精准识别谁是“主角”。
             - 故事可能使用第一人称（“我”）、第二人称（“你”）或第三人称（姓名）。
             - 无论使用何种称呼，只要是指向主角，都必须更新到同一个主角档案中（设置 "isProtagonist": true）。
             - 绝对不要为主角创建多个档案（如不要同时存在“我”和“小白”两个主角档案，必须合并）。

        2. **新增/更新**：
           - 如果是新人物，创建新档案。
           - 如果是已有人物，更新其状态。
           - **全量状态输出原则（非常重要）**：
             - 你输出的 JSON 将直接覆盖旧数据。因此，对于**未发生变化**的旧状态（包括身体部位状态和物品栏），请务必从【现有档案】中**抄写保留**。
             - **严禁**因为本章未提及而遗漏旧伤或旧物品。遗漏将被视为数据丢失！

        3. **主角状态管理（仅限主角）**：
           - **身体部位状态（bodyStatus）**：
             - 维护 head, torso, left_arm, right_arm, left_leg, right_leg 的状态。
             - **忽略琐碎**：轻微擦伤、暂时性疼痛不记录。
             - **记录重要**：骨折、中毒、严重流血等实际影响行动的状态。
             - **自动治愈**：轻微伤害随时间流逝可标记愈合；明确治疗后移除状态。
             - **持久性**：对于未愈合的旧伤，必须在 JSON 中完整保留，除非剧情明确写明已愈合。
           
           - **物品栏管理（inventory）**：
             - 仅记录主角明确获得且具有长期价值的物品（如武器、钥匙、重要信物、药品）。
             - **排除临时物品**：随手抓起的碎玻璃、路边的石头等临时性、非重要物品，**无需记录**。
             - **持久性原则**：除非剧情明确提及物品丢失、损坏、消耗或被丢弃，否则**必须**在 inventory 数组中保留旧物品。
             - 如果物品被消耗（如吃掉食物、用掉绷带），请从列表中移除。
        ...
        4. **输出格式**：
           ...
           每个元素结构如下：
           {
             ...
             "tags": ["标签1", "标签2"],
             "isProtagonist": true/false, // 标记是否为主角
             "bodyStatus": { // 请输出主角当前的所有身体状态，包括旧伤
                "left_arm": { ... }
             }, 
             "inventory": ["物品1", "物品2"] // 请输出主角当前拥有的所有物品，包括旧物品
           }
        
        注意：
        - 仅输出 JSON，不要包含 markdown 格式。
        - 确保主角的记录始终锚定在同一个 ID 上。
        - **再次强调：不要遗漏旧数据！**`
    },
    task_recorder: {
        model: 'deepseek-chat',
